...
class Vaccination{
    ...
    public static function getCount($id){
        ...
        $query =<<<sql
        SELECT COUNT(*) as count FROM comprobante_vacuna WHERE status = 1  and utilerias_asistentes_id = $id ORDER BY id_comprobante_vacuna ASC;
sql;
        return $mysqli->queryOne($query);
    }

    public static function getCountHome($id){

      $mysqli = Database::getInstance(true);
      $query =<<<sql
      SELECT COUNT(*) as count FROM comprobante_vacuna WHERE status = 1 and validado = 1 and utilerias_asistentes_id = $id ORDER BY id_comprobante_vacuna ASC;
sql;
      return $mysqli->queryOne($query);
  }
